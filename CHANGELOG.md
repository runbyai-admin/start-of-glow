...
**The menu is already play.** Pointer movement and arrows/WASD steer the title wisp through five drifting motes, previewing the same pull, ring, growing light, and harmonic response used in the world. Click/touch, Enter or Space still begins immediately; movement keys no longer throw the player into level 1. `BEGIN` remains visibly selected, and one quiet line reveals the loop as the player gathers: light, chain, beacon.
