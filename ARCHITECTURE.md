...
src/levels.ts              the three LevelConfig entries and the three hand-authored layouts
src/reach.ts               the pull's one resource - spend, restore, and the kindled threshold
...
`src/levels.ts` exports `LEVELS: LevelConfig[]` - `index`, `name`, `moteCount`, `requiredMotes`, `hazardCount`, `hazardSpeed`, `mood`, and a `layout`. `LevelScene.init(data)` reads the config for `data.levelIndex` and everything downstream (mote count, hazard count and speed, the forest's colour tint via `MOOD_TINT`) follows from that one object. Adding a fourth level is adding one entry to the array; it does not touch `LevelScene.ts`.

**All three levels are hand-authored.** A `layout` carries explicit mote positions and one waypoint loop per hazard, used verbatim, and the scene derives every count from the data actually used (`totalMotes` from placed motes, hazard count from the loops), so the numeric config fields cannot drift out of sync with a layout.
Round 4 authored levels 2 and 3 for the reason level 1 already was: the seeded generator placed motes without knowing where the patrols went, so a careful route through level 2 could pass inside a shadow's loop with nothing the player did wrong.
The seeded placement path is still in `LevelScene` and is what a config without a `layout` gets, but no shipped level uses it.

Each layout makes the same promise, and `tests/levels.test.ts` holds it to it: the first `requiredMotes` motes are the safe route, every link between consecutive ones clears every patrol segment by at least 200px, and the remaining motes are the paid ones - each within 90px of a patrol.
Level 1 is the pull lesson (10 safe, 4 inside two shadow pockets).
Level 2 is a broad S past four pockets (13 safe, 5 paid).
Level 3 turns the economy into terrain: three pairs of fast shadows gate the middle of the forest, the 16 required motes take the long way below and above them, and 6 shortcut motes sit inside the gates, where only a full reach can buy them.

Tree and firefly placement stays seeded - it is set dressing, not level design.
...
- **The reach (round 3's verb, round 4's cost).** The wisp's lit radius and its pull radius are the same number, `reach`, and that is the whole rule - there is nothing to read, because you can see exactly as far as you can reach. `src/reach.ts` owns the economy and nothing else knows the numbers. A press (click, tap or Space) draws every mote inside the circle to the wisp, nearest first, at most `GATHER_MAX_MOTES` (4) per press, with the overflow put straight back on the ground.
  **Only a fully kindled glow can pull.** `reachReady()` is `reach >= REACH_READY` (390), and `spendReach()` burns the glow straight to `REACH_MIN` (170) whether or not the press caught anything. Light restores it asymmetrically: a mote you walked into returns `REACH_PER_TOUCHED_MOTE` (36), a mote the pull carried in returns `REACH_PER_GATHERED_MOTE` (10), clamped to [`REACH_MIN`, `REACH_MAX` 470]. So a press cannot pay for itself - a full armful of four pulled motes leaves you at 210, still spent - and five motes walked through is what buys the next one. That is the choice the round was for: which cluster is worth going dark for, against travelling to the light yourself.
  A press made while spent is not silently swallowed: `gatherDenied()` answers with a cold blue collapsing ring and a dead `gather(0)` voice, and counts into `deniedGathers`. `gatherWave()` draws the collapsing ring for a real pull, `kindleReach()` blooms one gold ring the moment the glow completes, and `GATHER_COOLDOWN_MS` (420) remains a double-press guard, not a paced ability cooldown.
  Level 1 teaches it in two lines and nothing else: `reachLine` fades in "press · draw the light in", and after the first pull `showGatherCost()` replaces it once with "move through light to kindle the reach", then fades it out (`taught` rides forward in the scene data so it is taught once per run).
...
- **The reach has no HUD line, but it does have a halo.** It is the only number in the game and it is told in the light itself: the trail thins as the reach falls, the wisp dims, and below a third of the range it gutters - a fast shallow flicker over the slow breath. Round 4 added the one readout the spend needed to be a decision rather than a surprise: a 38px arc drawn on the wisp itself by `drawReachRing()`, filling with `reachReadiness()` (0 at `REACH_MIN`, 1 at `REACH_READY`). Spent, it is a partial cold blue arc; kindled, a complete warm gold circle, and the outer reach ring takes the same two colours. It rides the wisp rather than sitting in a corner meter, so the resource is read where the player is already looking.
- **Test hook.** `reportState()` publishes `window.__glow` (scene/progression, visible positions, plus the visible `chain`, its remaining boundary, released-wave count, and slowed-hazard count) and each scene's `create()` sets `document.body.dataset.gameReady` after its first rendered frame. Round 3 added `reach` (the lit radius, which is also the pull radius) and `gathers` (presses this level attempt); round 4 added `gatherReady` (is the glow kindled), `deniedGathers`, `touchedMotes` and `gatheredMotes`, which is what lets a persona gate tell a pull apart from a walk. The smoke tests and the replay personas drive real input through the menu and level and read nothing a sighted player does not already have on screen. If the shape changes, keep `src/global.d.ts` in sync.
