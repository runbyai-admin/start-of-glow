...
Renders are serialised host-wide with `flock` on `/tmp/glow-replay.lock` - three panes asking for a video at once would otherwise thrash the same cores. Logic-only runs skip the lock.
