...
src/scenes/MenuScene.ts    playable title clearing - steer through five preview motes; click/touch/Enter/Space starts level 1
...
- **MenuScene.** A playable first clearing with no gate: pointer movement and arrows/WASD steer the wisp through five drifting motes, previewing the level's gather/chain/radiance response. Click/touch, Enter or Space calls the guarded `begin()` immediately; movement keys never start the game accidentally. The restrained mechanic line advances from “gather light” through “keep the chain” to “wake the beacon,” while `BEGIN` stays visibly selected.
